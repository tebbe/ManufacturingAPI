...
angular.module('AtlasPPS').controller('journalController',
    ['$scope', '$rootScope', 'localStorageService', '$location', 'notificationService', 'authService', 'ledgerService', 'transactionService', 'ngAuthSettings', 'DTOptionsBuilder', 'PpsConstant', '$window', 'reportSettings',
        function ($scope, $rootScope, localStorageService, $location, notificationService, authService, ledgerService, transactionService, ngAuthSettings, DTOptionsBuilder, PpsConstant, $window, reportSettings) {
            ...
            $scope.modelHeading = "Add Journal";
            ...
            $scope.Note = "";
            var transactionType = PpsConstant.TransactionType.Journal;
            ...
            $scope.removeTransactionDetailItem = function (tran) {
                if ($scope.selectedTransaction && $scope.selectedTransaction.TransactionDetail) {
                    for (var i = 0; i < $scope.selectedTransaction.TransactionDetail.length; i++) {
                        if ($scope.selectedTransaction.TransactionDetail[i].TranHeadId === tran.TranHeadId) {
            ...
            var getTransactionList = function (tranTypeId) {
                ...
                var promise = transactionService.getJournalTransactionList(fiscalYear, companyId, tranTypeId);
            ...
            $scope.selectTransaction = function (tran) {
                ...
                $scope.modelHeading = "Update Journal";
            ...
            $scope.updateTransaction = function () {
                ...
                var promise;
                if ($scope.isNew) {
                    promise = transactionService.saveJournalTransaction($scope.selectedTransaction);
                } else {
                    promise = transactionService.updateJournalTransaction($scope.selectedTransaction);
                }
                promise.then(function (response) {
                    authService.loadingOff();
            ...
            $scope.getVoucherDetail = function (tranNo,text) {
