...
angular.module('AtlasPPS').controller('ledgerRptController',
    ['$scope', '$rootScope', 'localStorageService', 'notificationService', '$location', 'authService', 'ledgerRptService', 'ngAuthSettings', 'PpsConstant', '$window', 'DTOptionsBuilder',
    function ($scope, $rootScope, localStorageService, notificationService, $location, authService, ledgerRptService, ngAuthSettings, PpsConstant, $window, DTOptionsBuilder) {
        ...
        $scope.GetLedger = function () {
            ...
            var promise = ledgerRptService.GetLedger(vm);
            ...
            if ($scope.ledgerList.Detail) {
                for (var item in $scope.ledgerList.Detail) {
                    if ($scope.ledgerList.Detail.hasOwnProperty(item)) {
                        totalDrAmount += $scope.ledgerList.Detail[item].DrAmount;
                        totalCrAmount += $scope.ledgerList.Detail[item].CrAmount;
                    }
                }
                $scope.TotalDrAmount = totalDrAmount;
                $scope.TotalCrAmount = totalCrAmount;
            }
        };
        $scope.PrintLedgerReport = function (id) {
            ...
            popupWin.document.write('<html><head><link rel="stylesheet" type="text/css" href="../../css/bootstrap.css" /></head>' +
                ...
                '<h3 class="font-bold"><u>Ledger Report</u></h3></div></div>' +
