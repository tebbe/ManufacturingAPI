...
angular.module('AtlasPPS').controller('salesReportController',
    ['$scope', '$rootScope', 'localStorageService', '$location', 'authService', 'salesService', 'ngAuthSettings', 'DTOptionsBuilder', 'PpsConstant', '$window', 'reportSettings',
        function ($scope, $rootScope, localStorageService, $location, authService, salesService, ngAuthSettings, DTOptionsBuilder, PpsConstant, $window, reportSettings) {
            var authData = localStorageService.get('authorizationData');
            $scope.salesTotalReportList = [];
            ...
            $scope.GetSalesReport = function () {
                ...
                var promise = salesService.getTotalSalesReportList(vm);
                promise.then(function (response) {
                    $scope.salesTotalReportList = response.salesTotalReportList;
                    var totalAmount = 0;
                    if ($scope.salesTotalReportList && $scope.salesTotalReportList.length > 0) {
                        for (var i = 0; i < $scope.salesTotalReportList.length; i++) {
                            totalAmount = totalAmount + $scope.salesTotalReportList[i].TotalAmount;
                        }
            ...
            $scope.PrintSalesReport = function (id) {
                ...
                popupWin.document.write('<html><head><link rel="stylesheet" type="text/css" href="../../css/bootstrap.css" /></head>' +
                    ...
                    '<h3 class="font-bold"><u>Total Sales Report</u></h3></div></div>' +
